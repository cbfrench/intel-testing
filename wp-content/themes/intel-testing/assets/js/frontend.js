...
jQuery(document).ready(function ($) {
    ...
    const cleanAnimatedElements = () => {
        $(".scroll__start-hidden:not(.scroll__enabled)").removeClass(
            (index, className) => {
    ...
    const startAnimationImmediatelyOnElement = (
        element,
        animationData,
        lifecycle
    ) => {
    ...
    const prepareElement = (element) => {
        ...
        classes.forEach((c) => {
            if (c.startsWith("animate__")) {
                ...
                data.entrance.rootMargin = `0px 0px ${c.replace(
                    "scroll__offset__",
                    ""
                )}px 0px`;
        ...
        $(element).removeClass(
            [...animate_classes, "scroll__offset__", "scroll__threshold__"].join(" ")
        );
    ...
    const onEntrance = (element) => {
        ...
        startAnimationImmediatelyOnElement(
            element,
            $(element).attr("data-animation-entrance"),
            "entering"
        );
    ...
    const initScrollingElements = () => {
        ...
        $(".scroll__enabled").each(function () {
            ...
            // Entrance Trigger
            new IntersectionObserver((ents) => { ents.forEach((e) => { if (e.isIntersecting) onEntrance(e.target) }) }, data.entrance).observe(this);

            // Reset Trigger
            new IntersectionObserver((ents) => {
                ents.forEach((e) => {
                    if (!e.isIntersecting && $(e.target).hasClass("scroll__repeat")) { onReset(e.target); }
                });
            }, { rootMargin: "0px", threshold: 0 }).observe(this);
        });
    };
    ...
    $('.moon').animate({opacity: 1}, 300);
    $('.earth').animate({opacity: 1}, 300);
